...
var PrintPDF = function (options) {
	...
	var validate = function (formData, jqForm, params) {
		...
		if (typeof(mod_measure_RX) !== "undefined") {
			...
			updateFormField(formData, "measured_y_values", tmp_y);
                ...
                if ("undefined" !== typeof GlobalPrintableGeometries){
		...
		if ($("img.mapSymbol").length > 0) {
		    ...
		    var imgPos = $("img.mapSymbol").position();
		    var imgWidth = $("img.mapSymbol").attr("width");
		    var imgHeight = $("img.mapSymbol").attr("width");		
                    var permanentImg = imgSrc + "___" + imgPos.left + "___" + imgPos.top + "___" + imgWidth + "___" + imgHeight;
	...
	var showResult = function (res, text) {
		if (text == 'success') {
            ...
            var url = stripslashes(res.outputFileName);
            if (document.URL.indexOf("geoport-hro.de") != -1) {
                url = url.replace(/\/geoport\/offen/g, "");
                url = url.replace(/172.30.6.160/g, "www.geoport-hro.de");
            }
            else if (document.URL.indexOf("geo.sv.rostock.de") != -1) {
                url = url.replace(/\/geoport\/privat/g, "/geoport");
                url = url.replace(/172.30.6.160/g, "geo.sv.rostock.de");
			...
			if ($.browser.msie) {
 				$('<div></div>')
					...
					.append($('<a style="font-family:Verdana;color:#006cb7;text-decoration:none">PDF</a>')
						.attr('href', url)
				...
				window.frames[myId + "_frame"].location.href = url;
			}
			showHideWorking("hide");
	...
	var buildForm = function () {
		...
		for (var item in actualConfig.controls) {
			...
			if (element.type != "hidden") {
				...
				str += '<label class="print_label" style="cursor:text;" for="'+element.id+'">'+element.label+'</label>\n';
			} else {
				str += '<div class="print_option_dyn" style="display:none;">\n';	
			}
			switch (element.type) {
				case "text":
					str += '<input type="'+element.type+'" name="'+element.id+'" id="'+element.id+'" size="'+element.size+'" title="Eingabefeld für '+element.label+'"><br>\n';
					break;
				case "hidden":
					str += '<input type="'+element.type+'" name="'+element.id+'" id="'+element.id+'" title="Eingabefeld für '+element.label+'">\n';
					break;				
				case "textarea":
					str += '<textarea id="'+element.id+'" name="'+element.id+'" size="'+element.size+'" title="Eingabefeld für '+element.label+'"></textarea><br>\n';
					break;				
				case "select":
					str += '<select style="cursor:pointer;" id="'+element.id+'" name="'+element.id+'" size="1" title="Auswahlfeld für '+element.label+'">\n';
	...
	var buildConfigSelector = function () {
		...
		str += '</select>Druckrahmen<img id="printPDF_handle" style="cursor:pointer;margin-left:40px;width:20px;height:20px;vertical-align:middle;" src="../print/img/shape_handles.png" title="<?php echo htmlentities(_mb("Druckrahmen ein-/ausblenden"), ENT_QUOTES, "UTF-8");?>">\n';
		if (str) {
