...
$apiLogoLo = Mapbender::session()->get("layout_logo_lo");
$apiLogoCe = Mapbender::session()->get("layout_logo_ce");
$apiLogoRo = Mapbender::session()->get("layout_logo_ro");
...
echo "var apiHeaderImgArray = {";
echo " 'layout_logo_lo' : '" . $apiLogoLo . "',";
echo " 'layout_logo_ce' : '" . $apiLogoCe . "',";
echo " 'layout_logo_ro' : '" . $apiLogoRo . "'";
echo "};";
...
Mapbender.events.setBackgroundIsReady.register(function () {
	if (apiBackgroundWms !== '') {
		var newBackgroundWms = parseInt(apiBackgroundWms, 10);
...
Mapbender.events.init.register(function () {
    for (var id in apiHeaderImgArray) {
        var currentImg = apiHeaderImgArray[id];
        if (currentImg != '') {
            // matches a relative link
            var pattern = /^\.\.(\/[-\w]+)+\/[-\w]+\.\w+$/;
            if (currentImg.match(pattern)) {
                    $("#"+id).attr("src", currentImg);
            }
            else {
                    new Mb_warning("not a valid image src: " + currentImg);
            }
        }
    }
});
